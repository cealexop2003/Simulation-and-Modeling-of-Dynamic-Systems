...
[t, X] = ode45(@(t,x) lyap_mixed_dynamics(t,x,m_true,b_true,k_true,...
    a, gamma_m, gamma_b, gamma_k, theta_m1, theta_m2, u_type), ...
    tspan, x0, opts);
...
% --- Συνάρτηση Δυναμικής ---
function dx = lyap_mixed_dynamics(t, x, m, b, k, a, gamma_m, gamma_b, gamma_k, tm1, tm2, u_type)
    ...
    % --- Φίλτρα ---
    ddz0 = a^2 * x1 - 2*a*dz0 - a^2*z0;
    ddz1 = a^2 * x2 - 2*a*dz1 - a^2*z1;
    ddvu = a^2 * u  - 2*a*dvu - a^2*vu;

    % --- Γραμμικός συνδυασμός ex1/ex2 για m̂ ---
    alpha = 0.8;
    beta  = 0.2;
    e_m = alpha * ex2 + beta * ex1;

    % --- Gradient-based ενημέρωση παραμέτρων ---
